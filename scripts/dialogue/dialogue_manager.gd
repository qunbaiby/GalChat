...
func _on_script_background_requested(bg_path: String, fade_time: float) -> void:
	var tex = load(bg_path)
	if tex:
		# 尝试寻找 BackgroundLayer
		var bg_node = get_node_or_null("BackgroundLayer")
		if not bg_node:
			bg_node = get_node_or_null("BedroomMorning")
		
		if bg_node and bg_node is TextureRect:
			if fade_time > 0:
				var tween = create_tween()
				tween.tween_property(bg_node, "modulate:a", 0.0, fade_time / 2.0)
				tween.tween_callback(func(): bg_node.texture = tex)
				tween.tween_property(bg_node, "modulate:a", 1.0, fade_time / 2.0)
			else:
				bg_node.texture = tex
	script_engine.resume()
...
func _on_end_chat_pressed() -> void:
	...
	var prompt = "【系统提示：玩家想要结束本次对话。请根据你们当前的关系阶段（Stage %d），并结合以下刚才聊天的上下文，给出自然的告别反应。注意：不要输出这段系统提示，直接以%s的口吻说话。】\n[近期聊天上下文]\n%s" % [GameDataManager.profile.current_stage, char_name, recent_history]
