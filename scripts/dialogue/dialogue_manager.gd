...
func _on_script_dialogue_requested(speaker: String, content: String, mood: String, presentation: Dictionary = {}) -> void:
	var portrait_speaker = str(presentation.get("character", speaker))
	...
	if script_engine.use_story_portraits() and bool(presentation.get("exit", false)) and character_layer and character_layer.has_method("hide_story_character"):
		var exit_animation = str(presentation.get("exit_animation", "fade_out")).strip_edges()
		character_layer.hide_story_character(portrait_speaker, exit_animation if exit_animation != "" else "fade_out")
