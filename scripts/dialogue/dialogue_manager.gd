...
func _ready() -> void:
    ...
    if dialogue_panel.has_node("EndChatButton"):
        dialogue_panel.get_node("EndChatButton").hide()
    ...
    if affection_btn: affection_btn.pressed.connect(_on_affection_pressed)
    ...
    while true:
        ...
        if player_info_popup_event != null:
            ...
            if popup_scene:
                ...
                # 当信息提交后，更新档案属性 (记忆已经在 player_info_popup 内部保存)
