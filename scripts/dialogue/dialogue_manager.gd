...
func _ready() -> void:
    ...
    if gift_btn: gift_btn.pressed.connect(_on_gift_pressed)
    if voice_record_btn:
...
func _on_script_dialogue_requested(speaker: String, content: String, mood: String) -> void:
    ...
    await _show_message_async(content, char_name, true)
        ...
        if bg_node and bg_node is TextureRect:
            if duration > 0:
                ...
                if transition_type == "blur":
                    # Simulate a blur transition using alpha and scale if no shader is attached
                    # Or just use a simple fade out / scale / fade in if shader is too complex to setup dynamically
                    ...
                    tween.tween_property(bg_node, "position:x", original_x - get_viewport_rect().size.x, duration / 2.0).set_trans(Tween.TRANS_QUAD)
                    tween.tween_callback(func(): 
                        bg_node.texture = tex
                        bg_node.position.x = original_x + get_viewport_rect().size.x
                    )
                    ...
                    tween.tween_property(bg_node, "position:y", original_y - get_viewport_rect().size.y, duration / 2.0).set_trans(Tween.TRANS_QUAD)
                    tween.tween_callback(func(): 
                        bg_node.texture = tex
                        bg_node.position.y = original_y + get_viewport_rect().size.y
                    )
