...
func _ready() -> void:
    ...
    vision_http = HTTPRequest.new()
    vision_http.timeout = 30.0
    add_child(vision_http)
    vision_http.request_completed.connect(_on_vision_completed)
...
func generate_dynamic_topics(prompt: String, callback: Callable) -> void:
    ...
    var headers = [
        "Content-Type: application/json",
        "Authorization: Bearer " + GameDataManager.config.api_key
    ]
...
func generate_npc_event_dialogue(npc_id: String, event_desc: String) -> void:
    ...
    # 解析事件详情，构建当前事件描述
    var system_prompt = GameDataManager.prompt_manager.build_npc_event_prompt(npc_name, personality, protagonist_name, stage, stage_title, event_desc)
    
    if system_prompt.is_empty():
        # Fallback in case file read fails
        system_prompt = "【系统设定】\n你扮演的角色是：%s。\n你的性格特征和说话风格是：%s。\n注意：在这个世界里，你现在面对的是游戏世界中的少女【%s】。\n你当前与少女【%s】的情感羁绊处于【阶段%d：%s】。请严格根据这个情感阶段对她表现出相应的态度。\n\n【当前事件】\n%s\n\n【任务要求】\n请结合你的性格、情感阶段以及当前发生的事件，作出非常符合你人设的回复。注意：\n1. 所有的动作、神态、心理描写，必须且只能使用全角或半角的圆括号 () 包裹。\n2. 绝对禁止使用星号 *、中括号 []、波浪号 ~ 等其他任何符号来表示动作或情绪。\n3. 直接输出台词和圆括号动作，不要包含任何旁白。\n4. 语气必须严格符合当前对【%s】的情感阶段，且符合现代日常世界观（不要出现魔幻、修仙、系统、穿越等出戏话题）。\n5. 回复可以是多句话，以表达完整的情感和意思，如果有多句可以自然换行。像游戏里的即时互动反馈一样自然流畅。" % [npc_name, personality, protagonist_name, protagonist_name, stage, stage_title, event_desc, protagonist_name]
...
func _process_diary_illustration(diary_entry: Dictionary) -> void:
    ...
    var on_success = func(diary_id: String, local_path: String, metadata: Dictionary):
    ...
    var on_failed = func(diary_id: String, error_msg: String):
