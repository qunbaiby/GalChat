                    ...
                    if parse_json.parse(json_str) == OK:
                        ...
                        if data is Dictionary and data.has("operations") and data["operations"] is Array:
                            ...
                            for op in data["operations"]:
                                ...
                                if action == "ADD":
                                    await GameDataManager.memory_manager.add_memory(layer, content, _pending_memory_context)
                                    plain_text_changes += "新增记忆: %s\n" % content
                                elif action == "UPDATE":
                                    var success = await GameDataManager.memory_manager.update_memory(layer, id, content, _pending_memory_context)
                                    if success:
