...
@onready var mood_option: OptionButton = $"CenterContainer/Panel/VBoxContainer/TabContainer/状态与表情/HBoxContainer/MoodOption"
...
func _init_mood_options() -> void:
    mood_option.clear()
    var index = 0
    for expression_id in GameDataManager.expression_system.all_expression_ids:
        var config = GameDataManager.expression_system.expression_configs[expression_id]
        var display_text = "%s (%s)" % [config.get("name", expression_id), expression_id]
...
func show_panel() -> void:
    ...
    var expression_id = profile.current_expression
    var idx = GameDataManager.expression_system.all_expression_ids.find(expression_id)
    if idx >= 0:
...
func _on_mood_selected(index: int) -> void:
    if index >= 0 and index < GameDataManager.expression_system.all_expression_ids.size():
        var expression_id = GameDataManager.expression_system.all_expression_ids[index]
        GameDataManager.profile.update_expression(expression_id)
        mood_changed.emit(expression_id)
...
func _on_set_trait_pressed() -> void:
    ...
    if selected_idx >= 0 and selected_idx < trait_map.size():
        ...
        if profile:
            ...
            # 如果档案面板开着，也刷新它
            var main_scene = get_tree().current_scene
            if main_scene and main_scene.has_node("UIPanel/MobileInterface/PhonePanel/ArchivePanel"):
                var archive = main_scene.get_node("UIPanel/MobileInterface/PhonePanel/ArchivePanel")
                if archive.visible and archive.has_method("_on_visibility_changed"):
                    archive._on_visibility_changed()
