...
func _on_poll_timer_timeout() -> void:
    ...
    if GameDataManager.config.pet_enable_hourly_chime:
        _check_hourly_chime()
    if GameDataManager.config.pet_enable_app_observe:
        _check_active_window()
    if GameDataManager.config.pet_enable_afk_greeting:
        _check_afk_state()
...
func _check_active_window() -> void:
    ...
    if _time_since_last_switch >= observe_time:
        ...
        # 尝试截图 (优先截取当前活动窗口)
        var base64_image = ""
        # 强制添加判断：只有在全局配置中开启了视觉，才调用截图和多模态 API
...
func _on_main_window_pressed() -> void:
    ...
    var root_window = get_tree().root.get_window()
    root_window.show()
