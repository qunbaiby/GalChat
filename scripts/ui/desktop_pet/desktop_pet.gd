...
func _check_active_window() -> void:
	...
	if _time_since_last_switch >= 10.0 and _last_chatted_app != app_identifier:
		if current_tick - _last_reaction_tick < 60000:
			# 60秒内有过对话，稍后再尝试
			print("[DesktopPet Debug] Cooldown active, skipping proactive chat.")
			return
...
func _process_next_bubble() -> void:
	...
	# Wait for typewriter to finish + a short pause
