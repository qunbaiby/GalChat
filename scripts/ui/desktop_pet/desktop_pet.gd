...
@onready var tool_mode_chip: Label = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/ModeChip") as Label
...
@onready var record_entry_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/ToolButtonsRow/RecordEntryButton") as Button
@onready var close_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/ToolButtonsRow/CloseButton") as Button
@onready var pomodoro_toggle_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/PrimaryActionsRow/PomodoroToggleButton") as Button
@onready var mood_panel: PanelContainer = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/MoodPanel") as PanelContainer
@onready var embedded_music_host: VBoxContainer = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn") as VBoxContainer
@onready var music_playlist_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/MusicControlsRow/MusicPlaylistButton") as Button
@onready var music_prev_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/MusicControlsRow/MusicPrevButton") as Button
@onready var music_play_pause_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/MusicControlsRow/MusicPlayPauseButton") as Button
@onready var music_next_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/MusicControlsRow/MusicNextButton") as Button
@onready var music_volume_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/MusicControlsRow/MusicVolumeButton") as Button
@onready var music_shuffle_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/MusicControlsRow/MusicShuffleButton") as Button
@onready var music_repeat_button: Button = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/MusicControlsRow/MusicRepeatButton") as Button
@onready var music_progress: ProgressBar = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/BottomSection/BottomSectionMargin/MusicColumn/MusicProgress") as ProgressBar
@onready var tool_week_label: Label = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/TopRow/LeftStack/ClockCard/ClockMargin/ClockVBox/ClockTopRow/WeekBadge/WeekLabel") as Label
@onready var tool_time_label: Label = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/TopRow/LeftStack/ClockCard/ClockMargin/ClockVBox/ClockTopRow/TimeLabel") as Label
@onready var tool_date_label: Label = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/TopRow/LeftStack/ClockCard/ClockMargin/ClockVBox/ClockBottomRow/DateBadge/DateLabel") as Label
@onready var tool_month_label: Label = get_node_or_null("Control/QuickToolsPanel/Canvas/ShellMargin/ShellRoot/ContentMargin/ContentVBox/TopRow/LeftStack/ClockCard/ClockMargin/ClockVBox/ClockBottomRow/MonthBadge/MonthLabel") as Label
...
var _music_playlist_popup_instance: Control = null
var _music_volume_popup: PanelContainer = null
...
var _mood_value_label: Label = null
var _mood_hint_label: Label = null
...
enum CompactPlayMode { LOOP_LIST = 0, SHUFFLE = 1, REPEAT_ONE = 2 }
var _compact_music_mode: CompactPlayMode = CompactPlayMode.REPEAT_ONE
...
func _ready() -> void:
	...
	_setup_mood_panel_widget()
	_ensure_embedded_music_player()
	_connect_profile_ui_signals()
	...
	if record_entry_button:
		record_entry_button.pressed.connect(_on_record_entry_pressed)
...
func _connect_profile_ui_signals() -> void:
...
func _setup_mood_panel_widget() -> void:
	if mood_panel == null:
		return
	for child in mood_panel.get_children():
		child.queue_free()
	var shell_style := StyleBoxFlat.new()
	shell_style.bg_color = Color(1, 1, 1, 0.88)
	...
	shell_style.border_color = Color(0.82, 0.88, 0.92, 0.95)
	shell_style.corner_radius_top_left = 16
	shell_style.corner_radius_top_right = 16
	shell_style.corner_radius_bottom_right = 16
	shell_style.corner_radius_bottom_left = 16
	mood_panel.add_theme_stylebox_override("panel", shell_style)
	var margin := MarginContainer.new()
	margin.name = "MoodMargin"
	margin.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	margin.add_theme_constant_override("margin_left", 10)
	margin.add_theme_constant_override("margin_top", 6)
	margin.add_theme_constant_override("margin_right", 10)
	margin.add_theme_constant_override("margin_bottom", 6)
	mood_panel.add_child(margin)
	...
	top_row.name = "MoodTopRow"
	top_row.add_theme_constant_override("separation", 6)
	root.add_child(top_row)
	_mood_title_label = Label.new()
	_mood_title_label.name = "MoodTitle"
	_mood_title_label.text = "心情"
	_mood_title_label.add_theme_font_size_override("font_size", 14)
	top_row.add_child(_mood_title_label)
	_mood_emoji_label = Label.new()
	_mood_emoji_label.name = "MoodEmoji"
	_mood_emoji_label.add_theme_font_size_override("font_size", 14)
	top_row.add_child(_mood_emoji_label)
	_mood_name_label = Label.new()
	_mood_name_label.name = "MoodName"
	_mood_name_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_mood_name_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	_mood_name_label.add_theme_font_size_override("font_size", 14)
	top_row.add_child(_mood_name_label)
	...
	bg_style.bg_color = Color(0.9, 0.93, 0.95, 1.0)
	bg_style.corner_radius_top_left = 8
	bg_style.corner_radius_top_right = 8
	bg_style.corner_radius_bottom_right = 8
	bg_style.corner_radius_bottom_left = 8
	var fill_style := StyleBoxFlat.new()
	fill_style.bg_color = Color(0.31, 0.82, 0.58, 1.0)
	...
	bottom_row.name = "MoodBottomRow"
	bottom_row.add_theme_constant_override("separation", 6)
	root.add_child(bottom_row)
	_mood_hint_label = Label.new()
	_mood_hint_label.name = "MoodHint"
	_mood_hint_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_mood_hint_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_mood_hint_label.add_theme_font_size_override("font_size", 11)
	bottom_row.add_child(_mood_hint_label)
	_mood_value_label = Label.new()
	_mood_value_label.name = "MoodValue"
	_mood_value_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	_mood_value_label.add_theme_font_size_override("font_size", 11)
	bottom_row.add_child(_mood_value_label)

func _update_mood_panel_ui() -> void:
	if _mood_name_label == null or _mood_value_label == null or _mood_hint_label == null:
		return
	if GameDataManager == null or GameDataManager.profile == null or GameDataManager.mood_system == null:
		if _mood_emoji_label:
			_mood_emoji_label.text = ""
		_mood_name_label.text = "未知"
		_mood_value_label.text = "-- / 100"
		_mood_hint_label.text = "当前暂无心情数据"
		if _mood_bar:
			_mood_bar.value = 0.0
		return
	...
	var palette := _get_tool_mood_palette(mood_id)
	if _mood_title_label:
		_mood_title_label.add_theme_color_override("font_color", palette.get("accent_color", Color(0.13, 0.76, 0.70, 1.0)))
	if _mood_emoji_label:
		_mood_emoji_label.text = mood_emoji
	if _mood_name_label:
		_mood_name_label.text = mood_name
		_mood_name_label.add_theme_color_override("font_color", palette.get("name_color", Color(0.16, 0.18, 0.22, 1.0)))
	if _mood_value_label:
		_mood_value_label.text = "%d / 100" % mood_value
		_mood_value_label.add_theme_color_override("font_color", palette.get("value_color", Color(0.16, 0.18, 0.22, 1.0)))
	if _mood_hint_label:
		_mood_hint_label.text = _build_tool_mood_hint(mood_id)
		_mood_hint_label.add_theme_color_override("font_color", palette.get("hint_color", Color(0.38, 0.42, 0.48, 1.0)))
	if _mood_bar:
		_mood_bar.value = mood_value
		var fill_style := _mood_bar.get_theme_stylebox("fill") as StyleBoxFlat
		if fill_style:
			fill_style.bg_color = palette.get("bar_color", Color(0.31, 0.82, 0.58, 1.0))

func _build_tool_mood_hint(mood_id: String) -> String:
	match mood_id:
		"broken":
			return "需要安抚"
		"low":
...
func _get_tool_mood_palette(mood_id: String) -> Dictionary:
	match mood_id:
		"broken":
			return {
				"accent_color": Color(0.93, 0.47, 0.60, 1.0),
				"name_color": Color(0.80, 0.26, 0.40, 1.0),
				"value_color": Color(0.86, 0.32, 0.46, 1.0),
				"hint_color": Color(0.74, 0.36, 0.46, 1.0),
				"bar_color": Color(0.92, 0.36, 0.56, 1.0)
			}
		"low":
			return {
				"accent_color": Color(0.97, 0.67, 0.42, 1.0),
				"name_color": Color(0.76, 0.42, 0.12, 1.0),
				"value_color": Color(0.88, 0.50, 0.18, 1.0),
				"hint_color": Color(0.66, 0.44, 0.22, 1.0),
				"bar_color": Color(0.95, 0.60, 0.30, 1.0)
			}
		"pleasant":
			return {
				"accent_color": Color(0.37, 0.84, 0.64, 1.0),
				"name_color": Color(0.17, 0.60, 0.42, 1.0),
				"value_color": Color(0.23, 0.70, 0.50, 1.0),
				"hint_color": Color(0.25, 0.56, 0.44, 1.0),
				"bar_color": Color(0.31, 0.82, 0.58, 1.0)
			}
		"ecstatic":
			return {
				"accent_color": Color(0.33, 0.74, 0.98, 1.0),
				"name_color": Color(0.16, 0.46, 0.70, 1.0),
				"value_color": Color(0.22, 0.55, 0.82, 1.0),
				"hint_color": Color(0.24, 0.46, 0.64, 1.0),
				"bar_color": Color(0.28, 0.68, 0.96, 1.0)
			}
		_:
			return {
				"accent_color": Color(0.43, 0.55, 0.64, 1.0),
				"name_color": Color(0.20, 0.24, 0.28, 1.0),
				"value_color": Color(0.26, 0.30, 0.36, 1.0),
				"hint_color": Color(0.38, 0.42, 0.48, 1.0),
				"bar_color": Color(0.55, 0.68, 0.76, 1.0)
			}

func _ensure_embedded_music_player() -> void:
	if embedded_music_host == null:
		return
	_setup_compact_music_button(music_playlist_button, ICON_COVER, "打开播单")
	_setup_compact_music_button(music_prev_button, ICON_PREV, "上一首")
	_setup_compact_music_button(music_play_pause_button, ICON_PLAY, "播放")
	_setup_compact_music_button(music_next_button, ICON_NEXT, "下一首")
	_setup_compact_music_button(music_volume_button, ICON_VOLUME_FILL, "音量")
	_setup_compact_music_button(music_shuffle_button, ICON_ORDER_PLAY, "列表循环")
	_setup_compact_music_button(music_repeat_button, ICON_REPEAT_ONE, "单曲循环")
	if music_playlist_button and not music_playlist_button.pressed.is_connected(_on_embedded_music_playlist_pressed):
		music_playlist_button.pressed.connect(_on_embedded_music_playlist_pressed)
	if music_prev_button and not music_prev_button.pressed.is_connected(_on_embedded_music_prev_pressed):
		music_prev_button.pressed.connect(_on_embedded_music_prev_pressed)
	if music_play_pause_button and not music_play_pause_button.pressed.is_connected(_on_embedded_music_play_pause_pressed):
		music_play_pause_button.pressed.connect(_on_embedded_music_play_pause_pressed)
	if music_next_button and not music_next_button.pressed.is_connected(_on_embedded_music_next_pressed):
		music_next_button.pressed.connect(_on_embedded_music_next_pressed)
	if music_volume_button and not music_volume_button.pressed.is_connected(_on_embedded_music_volume_pressed):
		music_volume_button.pressed.connect(_on_embedded_music_volume_pressed)
	if music_shuffle_button and not music_shuffle_button.pressed.is_connected(_on_embedded_music_shuffle_pressed):
		music_shuffle_button.pressed.connect(_on_embedded_music_shuffle_pressed)
	if music_repeat_button and not music_repeat_button.pressed.is_connected(_on_embedded_music_repeat_pressed):
		music_repeat_button.pressed.connect(_on_embedded_music_repeat_pressed)
	if music_player and not music_player.finished.is_connected(_on_embedded_music_finished):
		music_player.finished.connect(_on_embedded_music_finished)
	_ensure_embedded_music_volume_popup()
	_ensure_embedded_music_playlist_popup()
	_refresh_music_panel_state()

func _setup_compact_music_button(button: Button, icon: Texture2D, tooltip: String) -> void:
	if button == null:
		return
	button.text = ""
	button.icon = icon
...
func _ensure_embedded_music_playlist_popup() -> Control:
	if is_instance_valid(_music_playlist_popup_instance):
		return _music_playlist_popup_instance
	_music_playlist_popup_instance = PLAYLIST_POPUP_SCENE.instantiate()
	var host := _get_overlay_root()
	if host == null:
		host = $Control
	host.add_child(_music_playlist_popup_instance)
	if _music_playlist_popup_instance.has_signal("close_requested"):
		_music_playlist_popup_instance.close_requested.connect(func(): pass)
	if _music_playlist_popup_instance.has_method("get_category_option"):
		var category_option: OptionButton = _music_playlist_popup_instance.get_category_option()
		if category_option:
			category_option.hide()
	if _music_playlist_popup_instance.has_method("get_import_btn"):
		var import_btn: Button = _music_playlist_popup_instance.get_import_btn()
		if import_btn:
			import_btn.hide()
	_build_embedded_music_playlist_ui()
	return _music_playlist_popup_instance

func _ensure_embedded_music_volume_popup() -> void:
	if is_instance_valid(_music_volume_popup):
		return
	var popup_panel := PanelContainer.new()
	popup_panel.name = "DesktopPetVolumePopup"
	popup_panel.visible = false
	popup_panel.top_level = true
	var popup_style := StyleBoxFlat.new()
	popup_style.bg_color = Color(0.98, 0.985, 0.99, 0.96)
	...
	popup_style.border_color = Color(0.87, 0.89, 0.93, 1)
	...
	popup_style.shadow_color = Color(0.57, 0.82, 0.76, 0.12)
	...
	slider.custom_minimum_size = Vector2(24, 110)
	slider.min_value = 0.0
	slider.max_value = 1.0
	slider.step = 0.05
	slider.value = GameDataManager.config.bgm_volume if GameDataManager and GameDataManager.config else 1.0
	slider.value_changed.connect(_on_embedded_music_volume_changed)
	margin.add_child(slider)
	_music_volume_slider = slider
	_music_volume_popup = popup_panel
	var host := _get_overlay_root()
	if host == null:
		host = $Control
	host.add_child(popup_panel)
...
func _show_floating_panel(key: String, panel: Control, move_target: Control, anchor_bottom: bool = false, prepare_only: bool = false) -> void:
	if panel == null:
		return
	...
	if preserve_layout and not prepare_only:
		if panel.has_method("refresh_layout"):
			panel.call_deferred("refresh_layout")
		elif panel.has_method("_update_popup_layout"):
			panel.call_deferred("_update_popup_layout")
	if prepare_only:
		return
	panel.show()
...
func _get_music_panel_instance() -> Control:
	_ensure_embedded_music_player()
	return _ensure_embedded_music_playlist_popup()
...
func _get_desktop_pet_history_panel_instance() -> Control:
	if is_instance_valid(_desktop_pet_history_panel_instance):
		return _desktop_pet_history_panel_instance
	_desktop_pet_history_panel_instance = DESKTOP_PET_HISTORY_PANEL_SCENE.instantiate()
	var wrapper := _ensure_floating_window("pet_history")
	wrapper.add_child(_desktop_pet_history_panel_instance)
	_desktop_pet_history_panel_instance.visible = false
	_desktop_pet_history_panel_instance.set_anchors_and_offsets_preset(Control.PRESET_TOP_LEFT)
	_desktop_pet_history_panel_instance.position = Vector2.ZERO
	var drag_handle: Control = _desktop_pet_history_panel_instance.get_node_or_null("RootMargin/RootVBox/TopBar") as Control
	if _desktop_pet_history_panel_instance.has_signal("back_requested"):
		_desktop_pet_history_panel_instance.back_requested.connect(func(): _close_floating_panel("pet_history"))
	if _desktop_pet_history_panel_instance.has_signal("play_voice_requested"):
		_desktop_pet_history_panel_instance.play_voice_requested.connect(_on_pet_history_play_voice_requested)
	_register_floating_panel_entry("pet_history", _desktop_pet_history_panel_instance, wrapper, _desktop_pet_history_panel_instance, drag_handle, true)
	_floating_panel_entries["pet_history"]["window_size_override"] = Vector2(960, 540)
	return _desktop_pet_history_panel_instance
...
func _update_panel_clock() -> void:
	...
	var weekday_raw: int = int(time_dict.get("weekday", 0))
	var month_raw: int = clampi(int(time_dict.get("month", 1)), 1, 12)
	var hour_value: int = clampi(int(time_dict.get("hour", 0)), 0, 23)
	var minute_value: int = clampi(int(time_dict.get("minute", 0)), 0, 59)
	var day_value: int = clampi(int(time_dict.get("day", 1)), 1, 31)
	var weekday_names := ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"]
	var month_names := ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"]
	var weekday_index := weekday_raw
	if weekday_raw >= 1 and weekday_raw <= 7:
		weekday_index = weekday_raw % 7
	weekday_index = clampi(weekday_index, 0, weekday_names.size() - 1)
	if tool_week_label:
		tool_week_label.text = weekday_names[weekday_index]
	if tool_time_label:
		tool_time_label.text = "%02d:%02d" % [hour_value, minute_value]
	if tool_date_label:
		tool_date_label.text = "%02d.%02d" % [month_raw, day_value]
	if tool_month_label:
		tool_month_label.text = month_names[month_raw - 1]
...
func _on_memory_button_pressed() -> void:
	...
	_show_floating_panel("memory", panel, move_target, false, true)
...
func _open_music_panel(force_open: bool = true) -> void:
	var popup_panel: Control = _ensure_embedded_music_playlist_popup()
	if popup_panel == null:
		return
	_build_embedded_music_playlist_ui()
	var should_show: bool = force_open or not popup_panel.visible
	popup_panel.visible = should_show
	if should_show and popup_panel.has_method("show_above_target"):
		var target: Control = embedded_music_host
		if music_playlist_button != null:
			target = music_playlist_button
		popup_panel.show_above_target(target)

func _refresh_music_panel_state() -> void:
	var bgm_volume := GameDataManager.config.bgm_volume if GameDataManager and GameDataManager.config else 1.0
	if music_playlist_button:
		music_playlist_button.icon = ICON_COVER
		music_playlist_button.tooltip_text = "打开播单"
	if music_prev_button:
		music_prev_button.icon = ICON_PREV
		music_prev_button.tooltip_text = "上一首"
	if music_next_button:
		music_next_button.icon = ICON_NEXT
		music_next_button.tooltip_text = "下一首"
	if music_play_pause_button:
		var playing_now := music_player != null and music_player.playing and not music_player.stream_paused
		music_play_pause_button.icon = ICON_PAUSE if playing_now else ICON_PLAY
		music_play_pause_button.tooltip_text = "暂停" if playing_now else "播放"
	if music_volume_button:
		music_volume_button.icon = ICON_VOLUME_FILL if bgm_volume >= 0.5 else ICON_VOLUME_LINE
		music_volume_button.tooltip_text = "音量 %.0f%%" % (bgm_volume * 100.0)
	if music_shuffle_button:
		music_shuffle_button.icon = ICON_SHUFFLE_FILL if _compact_music_mode == CompactPlayMode.SHUFFLE else ICON_ORDER_PLAY
		music_shuffle_button.tooltip_text = "随机播放" if _compact_music_mode == CompactPlayMode.SHUFFLE else "列表循环"
		music_shuffle_button.modulate = Color(0.35, 0.74, 0.98, 1.0) if _compact_music_mode == CompactPlayMode.SHUFFLE else Color.WHITE
	if music_repeat_button:
		music_repeat_button.icon = ICON_REPEAT_ONE if _compact_music_mode == CompactPlayMode.REPEAT_ONE else ICON_REPEAT_LIST
		music_repeat_button.tooltip_text = "单曲循环" if _compact_music_mode == CompactPlayMode.REPEAT_ONE else "列表循环"
		music_repeat_button.modulate = Color(0.98, 0.69, 0.32, 1.0) if _compact_music_mode == CompactPlayMode.REPEAT_ONE else Color.WHITE
	if music_progress:
		if music_player and music_player.stream and music_player.stream.get_length() > 0.0:
			music_progress.value = clampf(music_player.get_playback_position() / music_player.stream.get_length(), 0.0, 1.0)
		else:
			music_progress.value = 0.0
	if is_instance_valid(_music_volume_slider):
		_music_volume_slider.value = bgm_volume
	_update_embedded_music_playlist_ui()

func _build_embedded_music_playlist_ui() -> void:
	var popup_panel: Control = _ensure_embedded_music_playlist_popup()
	if popup_panel == null or not popup_panel.has_method("get_playlist_container"):
		return
	var playlist_container: VBoxContainer = popup_panel.get_playlist_container()
	...
	if tracks.is_empty():
		...
		return
	var current_index := _resolve_current_music_index(tracks)
	var is_playing_now := music_player != null and music_player.playing and not music_player.stream_paused
	for i in range(tracks.size()):
		var item_data: Dictionary = tracks[i]
		...
		item.setup(
			...
			i == current_index and is_playing_now,
		...
		if item.has_method("get_star_btn"):
			...
			if star_btn != null:
				star_btn.hide()
		item.item_clicked.connect(_on_embedded_music_playlist_item_clicked)

func _update_embedded_music_playlist_ui() -> void:
	if not is_instance_valid(_music_playlist_popup_instance) or not _music_playlist_popup_instance.has_method("get_playlist_container"):
		return
	var playlist_container: VBoxContainer = _music_playlist_popup_instance.get_playlist_container()
	var tracks: Array = MusicLibraryData.load_playlist_tracks()
	var current_index := _resolve_current_music_index(tracks)
	var is_playing_now := music_player != null and music_player.playing and not music_player.stream_paused
	for i in range(playlist_container.get_child_count()):
		...
		if item.has_method("set_playing"):
			item.set_playing(i == current_index and is_playing_now)

func _on_embedded_music_playlist_pressed() -> void:
	_consume_desktop_pet_ui_input()
	var popup_panel: Control = _ensure_embedded_music_playlist_popup()
	if popup_panel == null:
		return
...
func _on_embedded_music_playlist_item_clicked(index: int) -> void:
	var tracks: Array = MusicLibraryData.load_playlist_tracks()
	if tracks.is_empty():
		return
	var current_index := _resolve_current_music_index(tracks)
	if current_index == index and music_player and music_player.stream:
		_on_embedded_music_play_pause_pressed()
		return
	_play_music_by_index(index)

func _on_embedded_music_volume_pressed() -> void:
	_consume_desktop_pet_ui_input()
	_ensure_embedded_music_volume_popup()
	if not is_instance_valid(_music_volume_popup) or music_volume_button == null:
		return
	if _music_volume_popup.visible:
		_music_volume_popup.hide()
		return
	_update_embedded_music_volume_popup_position()
	_music_volume_popup.show()
	_music_volume_popup.move_to_front()

func _update_embedded_music_volume_popup_position() -> void:
	if not is_instance_valid(_music_volume_popup) or music_volume_button == null:
		return
	var popup_size := _music_volume_popup.get_combined_minimum_size()
	if popup_size == Vector2.ZERO:
		popup_size = Vector2(44, 130)
	var button_rect := music_volume_button.get_global_rect()
	_music_volume_popup.size = popup_size
	_music_volume_popup.global_position = Vector2(
		round(button_rect.position.x + (button_rect.size.x - popup_size.x) * 0.5),
		round(button_rect.position.y - popup_size.y - 8.0)
	)

func _on_embedded_music_volume_changed(value: float) -> void:
...
func _on_embedded_music_shuffle_pressed() -> void:
	_consume_desktop_pet_ui_input()
	if _compact_music_mode == CompactPlayMode.SHUFFLE:
		_compact_music_mode = CompactPlayMode.LOOP_LIST
	else:
		_compact_music_mode = CompactPlayMode.SHUFFLE
	_apply_compact_loop_mode_to_current_stream()
	_refresh_music_panel_state()

func _on_embedded_music_repeat_pressed() -> void:
	_consume_desktop_pet_ui_input()
	if _compact_music_mode == CompactPlayMode.REPEAT_ONE:
		_compact_music_mode = CompactPlayMode.LOOP_LIST
	else:
		_compact_music_mode = CompactPlayMode.REPEAT_ONE
	_apply_compact_loop_mode_to_current_stream()
	_refresh_music_panel_state()

func _on_embedded_music_finished() -> void:
	_on_embedded_music_next_pressed(true)

func _apply_compact_loop_mode_to_current_stream() -> void:
	if music_player == null or music_player.stream == null:
		return
	_apply_compact_loop_mode_to_stream(music_player.stream)

func _apply_compact_loop_mode_to_stream(stream: AudioStream) -> void:
	if stream == null:
		return
	if stream is AudioStreamOggVorbis:
		(stream as AudioStreamOggVorbis).loop = _compact_music_mode == CompactPlayMode.REPEAT_ONE
	elif stream is AudioStreamMP3:
		(stream as AudioStreamMP3).loop = _compact_music_mode == CompactPlayMode.REPEAT_ONE

func _play_music_by_index(track_index: int) -> void:
	var tracks: Array = MusicLibraryData.load_playlist_tracks()
	if tracks.is_empty():
		if ToastManager:
			ToastManager.show_system_toast("桌面播单还是空的，先去共创音乐面板加几首歌吧", Color.RED)
		return
	var target_index := posmod(track_index, tracks.size())
	var target_track: Dictionary = tracks[target_index]
	...
	if main_bgm and main_bgm.playing:
		main_bgm.stop()
	_apply_compact_loop_mode_to_stream(stream)
...
func _resolve_current_music_index(tracks: Array) -> int:
	if tracks.is_empty():
		return -1
	var current_track_id := ""
	if music_player:
		current_track_id = str(music_player.get_meta("music_track_id", ""))
	if current_track_id == "":
		return 0
	for index in range(tracks.size()):
		if str(tracks[index].get("id", "")) == current_track_id:
			return index
	return 0

func _on_embedded_music_prev_pressed() -> void:
	_consume_desktop_pet_ui_input()
	var tracks := MusicLibraryData.load_playlist_tracks()
	if tracks.is_empty():
		return
	var current_index := _resolve_current_music_index(tracks)
	if _compact_music_mode == CompactPlayMode.SHUFFLE:
		current_index = randi() % tracks.size()
	else:
		current_index = (current_index - 1 + tracks.size()) % tracks.size()
	_play_music_by_index(current_index)

func _on_embedded_music_next_pressed(natural_end: bool = false) -> void:
	_consume_desktop_pet_ui_input()
	var tracks := MusicLibraryData.load_playlist_tracks()
	if tracks.is_empty():
		return
	...
	if _compact_music_mode == CompactPlayMode.SHUFFLE:
		current_index = randi() % tracks.size()
	elif _compact_music_mode == CompactPlayMode.REPEAT_ONE and natural_end:
		pass
	else:
		current_index = (current_index + 1) % tracks.size()
	_play_music_by_index(current_index)

func _on_embedded_music_play_pause_pressed() -> void:
	_consume_desktop_pet_ui_input()
	var tracks := MusicLibraryData.load_playlist_tracks()
	if tracks.is_empty():
		return
	...
	elif music_player.stream and music_player.stream_paused:
		music_player.stream_paused = false
	else:
...
func _build_desktop_pet_history_meta(origin: String) -> Dictionary:
	return {
		"module": "desktop_pet",
		"subtype": origin
	}

func _append_desktop_pet_history_message(speaker: String, text: String, voice_cache_key: String = "", origin: String = CHAT_ORIGIN_SYSTEM) -> void:
...
func _build_desktop_pet_voice_cache_key(text: String) -> String:
	if TTSManager == null or GameDataManager == null or GameDataManager.config == null:
		return ""
...
func _on_pet_history_play_voice_requested(cache_key: String) -> void:
	if cache_key.strip_edges() == "":
		return
	var stream: AudioStream = TTSManager.load_cached_audio_by_key(cache_key)
...
func _on_record_entry_pressed() -> void:
	...
	if panel and panel.has_method("show_panel"):
		panel.show_panel()
	_show_floating_panel("pet_history", panel, panel)
...
func _trigger_touch_app_observe() -> bool:
	...
	if not _is_app_observe_allowed_by_policy(process_name, window_title):
		print("[桌宠应用观察][已拦截] process=%s | title=%s" % [process_name, window_title])
		return false
	...
	print("[桌宠应用观察][目标窗口] process=%s | title=%s | app_type=%s | display_name=%s | soft_reminder=%s" % [
		process_name,
		window_title,
		app_type,
		display_name,
	...
	if base64_image != "":
		print("[桌宠应用观察][视觉分支] image_size=%d | allow_capture=%s" % [base64_image.length(), str(allow_capture)])
	...
	print("[桌宠应用观察][文本分支] fallback_reason=%s | allow_capture=%s | vision_enabled=%s | has_vision_key=%s" % [
		fallback_reason,
		str(allow_capture),
		str(vision_enabled),
		str(has_vision_key)
...
func _is_self_observe_window(process_name: String, window_title: String) -> bool:
	...
	if title_lower.contains("godot engine"):
		return true
	# 只拦截真正的游戏自身窗口，避免把 Trae/VSCode 这类编辑器里的项目标题误判成自观察窗口。
	if process_lower.contains("galchat") and title_lower.contains("desktop_pet"):
		return true
	return false
...
func _get_app_observe_reply_guidance(app_type: String, soft_reminder: bool) -> String:
	...
	match app_type:
		...
		"网页浏览器":
			return "如果是网页，优先写成：先被主页面最刺眼的主题或氛围勾住一下，再顺势表达一点好奇、在意、轻酸或轻吐槽，最后把注意力落回玩家。不要先猜用途，不要上来问“是找参考吗/是在研究吗/是在收藏吗”。如果画面本身已经有很明确的商店页、成人标记、游戏名或平台信息，就顺着这些真实可见证据把在意写得更贴脸一点，不要只轻飘飘说一句“封面有点直白”。"
		"视频":
...
func _on_vision_completed(response: Dictionary) -> void:
	...
	if analysis_text != "":
		...
		print("[桌宠应用观察][分析结果] ", analysis_text)
		...
		_append_desktop_pet_history_message("char", text, _build_desktop_pet_voice_cache_key(text), CHAT_ORIGIN_TOUCH)
		display_bubble(text)
...
func _build_touch_observe_output_contract(visual_confirmed: bool) -> String:
	var lines: Array[String] = []
	var is_adult_scene := _is_touch_observe_adult_scene(_last_observed_analysis_text)
	if visual_confirmed:
		lines.append("【输出要求】只围绕分析里明确写出的刺激点开口，不补用途、不补动机、不补新剧情。")
	else:
		lines.append("【输出要求】只围绕窗口类型或可见线索开口，不要假装看到了具体画面。")
	if is_adult_scene and visual_confirmed:
		lines.append("先给第一反应，再把情绪落到玩家身上，最后补半句轻收尾。这类暧昧/成人向场景默认写成 3 个短句、3 段左右 [SPLIT]，不要偷懒只写一句。")
	else:
...
- 如果分析里明确提到了“仅限成人”“成人标记”“Steam 商店页”“游戏名”“发行商”这类真实可见信息，不要假装没看见。可以顺着这些证据把在意写得更贴脸，比如被那句成人标记刺一下，或意识到玩家已经点进具体商品页了，但仍然不要脑补他接下来一定会购买、下载或收藏。
...
- 这类场景默认至少走完三拍：先停顿或掩饰一下，再把在意落到玩家身上，最后补半句轻轻占位或嘴硬收尾。不要只给一拍反应。
...
func _normalize_bubble_chunk_text(text: String) -> String:
...
func _collapse_tool_panel_for_pet_interaction() -> void:
	if quick_tools_panel and quick_tools_panel.visible:
		_set_menu_visible(false)
