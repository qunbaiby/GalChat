...
@onready var player_info_open_btn: Button = $PhonePanel/MainMargin/RootHBox/LeftColumn/PlayerInfoCard/ProfileOpenBtn
...
@onready var player_title_lbl: Label = $PhonePanel/MainMargin/RootHBox/LeftColumn/PlayerInfoCard/PlayerInfoArea/ProfileMargin/ProfileVBox/HeaderRow/NameArea/TitleLabelPanel/TitleLabel
...
var luna_profile_panel_instance = null
var tutorial_panel_instance = null
var mail_panel_instance = null
...
func _ready() -> void:
	...
	player_info_open_btn.pressed.connect(_on_player_info_pressed)
...
func _refresh_player_info_card() -> void:
	...
	if one_line_intro == "":
		one_line_intro = "安静温柔，慢热却会把依赖和真心都悄悄交给你。"
	...
	if is_instance_valid(player_title_lbl):
		player_title_lbl.text = one_line_intro
...
func _on_player_info_pressed() -> void:
	var host: Node = get_tree().current_scene if get_tree().current_scene != null else get_tree().root
	if luna_profile_panel_instance == null:
		var panel_scene = load("res://scenes/ui/mobile/luna_profile_panel.tscn")
		luna_profile_panel_instance = panel_scene.instantiate()
		host.add_child(luna_profile_panel_instance)
		luna_profile_panel_instance.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	else:
		if luna_profile_panel_instance.get_parent() != host:
			luna_profile_panel_instance.reparent(host)
		host.move_child(luna_profile_panel_instance, -1)
	luna_profile_panel_instance.show_panel()
...
func _get_unread_count_for_char(char_id: String) -> int:
	var path = GameDataManager.get_character_save_path("mobile_chat_history.json", char_id)
	if not FileAccess.file_exists(path):
		return 0
	var file = FileAccess.open(path, FileAccess.READ)
	if file == null:
...
func _on_mail_app_pressed() -> void:
	var host: Node = get_tree().current_scene if get_tree().current_scene != null else get_tree().root
	if mail_panel_instance == null:
		var panel_scene = load("res://scenes/ui/mobile/mail_panel.tscn")
		mail_panel_instance = panel_scene.instantiate()
		host.add_child(mail_panel_instance)
		mail_panel_instance.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	else:
		if mail_panel_instance.get_parent() != host:
			mail_panel_instance.reparent(host)
		host.move_child(mail_panel_instance, -1)
	mail_panel_instance.show_panel()

func _on_tutorial_app_pressed() -> void:
	var host: Node = get_tree().current_scene if get_tree().current_scene != null else get_tree().root
	if tutorial_panel_instance == null:
		var panel_scene = load("res://scenes/ui/mobile/tutorial_panel.tscn")
		tutorial_panel_instance = panel_scene.instantiate()
		host.add_child(tutorial_panel_instance)
		tutorial_panel_instance.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	else:
		if tutorial_panel_instance.get_parent() != host:
			tutorial_panel_instance.reparent(host)
		host.move_child(tutorial_panel_instance, -1)
	tutorial_panel_instance.show_panel()
...
func _on_moments_cover_pick_requested() -> void:
	var host: Control = self if _wechat_overlay_only else phone_panel
	if album_panel_instance == null:
		var AlbumPanelObj = load("res://scenes/ui/mobile/album_panel.tscn")
		album_panel_instance = AlbumPanelObj.instantiate()
	else:
		if album_panel_instance.get_parent() != host:
			album_panel_instance.reparent(host)
	if album_panel_instance.get_parent() == null:
		host.add_child(album_panel_instance)
	album_panel_instance.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	host.move_child(album_panel_instance, -1)
