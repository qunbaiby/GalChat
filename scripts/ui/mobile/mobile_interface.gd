...
func _get_display_player_name(profile) -> String:
    var name_text := ""
    if profile:
        name_text = str(profile.player_name).strip_edges()
    if name_text == "" and GameDataManager.config:
        name_text = str(GameDataManager.config.player_name).strip_edges()
    if name_text == "":
        name_text = "未命名玩家"
    return name_text

func _get_display_player_title(profile) -> String:
    var title_text := ""
    if profile:
        title_text = str(profile.player_title).strip_edges()
    if title_text == "" and GameDataManager.config:
        title_text = str(GameDataManager.config.player_nickname).strip_edges()
    if title_text == "":
        title_text = "未设称呼"
    return title_text

func _refresh_player_info_card() -> void:
    var display_name := "未命名玩家"
    var display_title := "未设称呼"
    var gender_text := "未填写"
    var birthday_text := "未填写"
    var zodiac_text := "待补充"
    var mbti_text := "未选择"
    ...
    if GameDataManager.profile:
        ...
        display_name = _get_display_player_name(profile)
        display_title = _get_display_player_title(profile)

        gender_text = str(profile.player_gender).strip_edges()
        if gender_text == "" or gender_text == "其他":
            gender_text = "未填写"

        birthday_text = str(profile.player_birthday).strip_edges()
        if birthday_text == "":
            birthday_text = "未填写"

        zodiac_text = str(profile.player_zodiac).strip_edges()
        if zodiac_text == "":
            zodiac_text = "待补充"

        mbti_text = str(profile.player_mbti).strip_edges()
        if mbti_text == "":
            mbti_text = "未选择"

        avatar_texture = profile.get_player_avatar_texture()

    if is_instance_valid(player_name_lbl):
        player_name_lbl.text = display_name
    if is_instance_valid(player_title_lbl):
        player_title_lbl.text = "称呼  %s" % display_title
    if is_instance_valid(player_gender_lbl):
        player_gender_lbl.text = "性别：%s" % gender_text
    ...
    if is_instance_valid(player_mbti_lbl):
        player_mbti_lbl.text = "人格：%s" % mbti_text
