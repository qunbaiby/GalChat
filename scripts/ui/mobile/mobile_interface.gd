...
@onready var between_entry_panel: Panel = $PhonePanel/MainMargin/VBox/ListCards/BetweenEntry
@onready var between_entry_btn: Button = $PhonePanel/MainMargin/VBox/ListCards/BetweenEntry/BetweenEntryBtn
@onready var between_entry_title: Label = $PhonePanel/MainMargin/VBox/ListCards/BetweenEntry/BetweenEntryTitle
@onready var between_entry_text: Label = $PhonePanel/MainMargin/VBox/ListCards/BetweenEntry/BetweenEntryContent/BetweenEntryText
@onready var between_entry_icon: Label = $PhonePanel/MainMargin/VBox/ListCards/BetweenEntry/BetweenEntryIcon/BetweenEntryIconLabel
@onready var sms_btn: Button = $PhonePanel/MainMargin/VBox/ListCards/QuickEntries/ChatEntry/ChatEntryBtn
@onready var chat_entry_title: Label = $PhonePanel/MainMargin/VBox/ListCards/QuickEntries/ChatEntry/ChatEntryTitle
@onready var chat_entry_text: Label = $PhonePanel/MainMargin/VBox/ListCards/QuickEntries/ChatEntry/ChatEntryText
@onready var chat_entry_icon: Label = $PhonePanel/MainMargin/VBox/ListCards/QuickEntries/ChatEntry/ChatEntryIconLabel
@onready var moments_btn: Button = $PhonePanel/MainMargin/VBox/ListCards/QuickEntries/MomentsEntry/MomentsEntryBtn
@onready var moments_entry_title: Label = $PhonePanel/MainMargin/VBox/ListCards/QuickEntries/MomentsEntry/MomentsEntryTitle
@onready var moments_entry_text: Label = $PhonePanel/MainMargin/VBox/ListCards/QuickEntries/MomentsEntry/MomentsEntryText
@onready var moments_entry_icon: Label = $PhonePanel/MainMargin/VBox/ListCards/QuickEntries/MomentsEntry/MomentsEntryIconLabel
...
func _update_social_entry_labels() -> void:
	var sms_unread = _get_total_sms_unread_count()
	var moments_unread = MomentsManager.get_unread_moments_count() if MomentsManager and MomentsManager.has_method("get_unread_moments_count") else 0
	if is_instance_valid(chat_entry_title):
		chat_entry_title.text = "聊天" if sms_unread <= 0 else "聊天(%d)" % sms_unread
	if is_instance_valid(chat_entry_text):
		chat_entry_text.text = "暂时没有新的消息。" if sms_unread <= 0 else "有 %d 条未读消息。" % sms_unread
	if is_instance_valid(chat_entry_icon):
		chat_entry_icon.text = "✉"
	if is_instance_valid(moments_entry_title):
		moments_entry_title.text = "朋友圈" if moments_unread <= 0 else "朋友圈(%d)" % moments_unread
	if is_instance_valid(moments_entry_text):
		moments_entry_text.text = "没有新的互动提醒。" if moments_unread <= 0 else "有 %d 条新动态或评论。" % moments_unread
	if is_instance_valid(moments_entry_icon):
		moments_entry_icon.text = "☼"
	if contact_list_instance and contact_list_instance.visible and contact_list_instance.has_method("_load_contacts"):
		contact_list_instance._load_contacts()
	if moments_panel_instance and moments_panel_instance.visible and moments_panel_instance.has_method("refresh_list"):
...
func _configure_entry_cards() -> void:
	...
	if is_instance_valid(chat_entry_title):
		chat_entry_title.text = "聊天"
	if is_instance_valid(chat_entry_icon):
		chat_entry_icon.text = "✉"
	if is_instance_valid(moments_entry_title):
		moments_entry_title.text = "朋友圈"
	if is_instance_valid(moments_entry_icon):
		moments_entry_icon.text = "☼"
...
func _update_between_entry_card() -> void:
	...
	if is_instance_valid(between_entry_text):
		...
		if total <= 0:
			...
			between_entry_text.text = "已收录 %d 段回忆，回忆 %d 条，日记 %d 页，相片 %d 张。" % [total, memory_count, diary_count, photo_count]
