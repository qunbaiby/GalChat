...
@onready var level_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/HeroSection/HeartCard/HeartWrap/LevelLabel
@onready var stage_title_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/HeroSection/StageTitleLabel
@onready var points_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/HeroSection/PointsLabel
@onready var stage_progress_bar: ProgressBar = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/HeroSection/StageProgressBar
@onready var breakthrough_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/HeroSection/BreakthroughLabel
@onready var intimacy_value_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/StatsCard/Margin/StatsVBox/IntimacyRow/HBox/Value
@onready var trust_value_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/StatsCard/Margin/StatsVBox/TrustRow/HBox/Value
@onready var interaction_exp_value_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/StatsCard/Margin/StatsVBox/InteractionExpRow/HBox/Value
@onready var state_badge_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/SummaryCard/Margin/SummaryVBox/TitleRow/StateBadge/StateBadgeLabel
@onready var summary_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/SummaryCard/Margin/SummaryVBox/SummaryLabel
@onready var milestone_label: Label = $RootMargin/RootVBox/Scroll/Margin/ContentVBox/SummaryCard/Margin/SummaryVBox/MilestoneLabel
    ...
    if _map_name_cache.is_empty() and FileAccess.file_exists(MAP_DATA_PATH):
        ...
        if map_file:
            ...
            if map_json.parse(map_file.get_as_text()) == OK:
                ...
                for location_id in map_data.keys():
                    var location_info = map_data[location_id]
...
func _build_breakthrough_hint(profile, conf: Dictionary, progress_info: Dictionary) -> Dictionary:
    ...
    var exp_cost: int = int(conf.get("exp_cost", 0))
    ...
    if not progress_info.get("is_max_stage", false):
        ...
        parts.append("互动经验达到：%d" % exp_cost)
    else:
...
func update_ui(profile) -> void:
    ...
    intimacy_value_label.text = "+%d" % int(round(intimacy))
    trust_value_label.text = "+%d" % int(round(trust))
    interaction_exp_value_label.text = "%d" % int(profile.interaction_exp)
    ...
    $RootMargin/RootVBox/Scroll/Margin/ContentVBox/SummaryCard/Margin/SummaryVBox/TitleRow/StateBadge.add_theme_stylebox_override("panel", badge_style)
