...
@onready var basic_hbox: HBoxContainer = $MarginContainer/VBoxContainer/BasicStatsHBox
...
func _update_ui() -> void:
	...
	var idx = 0
	for vbox in basic_hbox.get_children():
		if vbox is VBoxContainer:
			for child in vbox.get_children():
				if child is PanelContainer: # e.g. Stat_PhysicalFitness
					...
					var val_label = child.find_child("ValLabel", true, false) as Label
					...
					if val_label: val_label.text = str(int(current_val))
