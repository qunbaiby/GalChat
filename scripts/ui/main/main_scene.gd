...
func _on_send_pressed() -> void:
	...
	if is_proactive_greeting and proactive_greeting_step == 0:
		proactive_greeting_step = 1
		var prompt = "【系统提示：这是本次主动问候的最后一轮回复，请根据玩家的话给出一个自然的回复（必须包含动作描写），回复完后本次问候即告结束。】\n玩家：" + text
		deepseek_client.send_chat_message_stream(prompt, "main_chat")
	else:
...
func _stream_worker_loop() -> void:
	...
	if is_proactive_greeting and proactive_greeting_step == 1:
		if audio_player and audio_player.playing:
			await audio_player.finished
		await get_tree().create_timer(1.0).timeout
		_close_chat_panel()
...
func _trigger_proactive_greeting() -> void:
	...
	var user_msg = GameDataManager.prompt_manager.build_proactive_greeting_prompt(GameDataManager.profile)
	deepseek_client.send_chat_message_stream(user_msg, "main_chat")
