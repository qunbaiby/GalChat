...
@onready var goal_title_label: Label = $UIPanel/GoalPanel/GoalMargin/GoalVBox/GoalTitle
...
var _goal_panel_tween: Tween = null
var _goal_panel_pending_reveal: bool = false
...
func _build_main_chat_meta(extra_data: Dictionary = {}) -> Dictionary:
	...
	if _current_main_chat_subtype == MAIN_CHAT_SUBTYPE_STORY_TOPIC and not _selected_main_story_topic.is_empty():
		var story_event_id := str(_selected_main_story_topic.get("event_id", "")).strip_edges()
		if story_event_id != "":
			meta["story_topic_event_id"] = story_event_id
		var source_type := str(_selected_main_story_topic.get("source_type", "")).strip_edges()
		if source_type != "":
			meta["story_topic_source_type"] = source_type
		var source_id := str(_selected_main_story_topic.get("source_id", "")).strip_edges()
		if source_id != "":
			meta["story_topic_source_id"] = source_id
	if not extra_data.is_empty():
		meta.merge(extra_data, true)
	return meta

func _get_main_chat_topic_manager() -> Node:
	return get_node_or_null("/root/MainChatTopicManager")

func _get_current_main_chat_character_id() -> String:
...
func _clear_expired_main_chat_topics() -> void:
	...
	if GameDataManager and GameDataManager.story_time_manager:
		current_day_offset = int(GameDataManager.story_time_manager.current_day_offset)
	manager.clear_expired_topics(current_day_offset)

func _get_active_story_topic_for_current_character() -> Dictionary:
	var manager := _get_main_chat_topic_manager()
	if manager == null or not manager.has_method("get_active_topic_for"):
		return {}
	var char_id := _get_current_main_chat_character_id()
	if char_id == "":
		return {}
	var topic_data: Variant = manager.get_active_topic_for(char_id)
...
func _consume_selected_story_topic_if_needed() -> void:
	if _selected_main_story_topic.is_empty():
		return
	var manager := _get_main_chat_topic_manager()
	if manager and manager.has_method("consume_active_topic"):
		var char_id := str(_selected_main_story_topic.get("character_id", _get_current_main_chat_character_id())).strip_edges().to_lower()
		if char_id != "":
...
func _update_affection_button_ui() -> void:
	...
	_update_affection_button_visibility()
...
func _update_goal_panel_ui() -> void:
	if not is_instance_valid(goal_panel) or not is_instance_valid(goal_title_label) or not is_instance_valid(goal_value_label):
		return
	var goal_manager := _get_goal_manager()
	if goal_manager == null or not goal_manager.has_method("get_primary_active_goal"):
		goal_title_label.text = ""
		goal_value_label.text = ""
		_hide_goal_panel()
		return
	var goal_data: Variant = goal_manager.get_primary_active_goal()
	if not (goal_data is Dictionary) or (goal_data as Dictionary).is_empty():
		goal_title_label.text = ""
		goal_value_label.text = ""
		_goal_panel_pending_reveal = false
		_hide_goal_panel()
		return
	_apply_goal_panel_text(goal_data as Dictionary)
	if goal_panel.visible:
		goal_panel.modulate.a = 1.0
		return
	if _goal_panel_pending_reveal and _can_play_goal_panel_reveal_animation():
		_show_goal_panel_with_fade()
		return
	goal_panel.show()
	goal_panel.modulate.a = 1.0

func _apply_goal_panel_text(goal_data: Dictionary) -> void:
	var title := str(goal_data.get("title", "")).strip_edges()
	var summary := str(goal_data.get("summary", "")).strip_edges()
	goal_title_label.text = title
	goal_value_label.text = summary

func _get_goal_manager() -> Node:
	return get_node_or_null("/root/GoalManager")

func _on_goals_updated() -> void:
	_update_goal_panel_ui()

func _on_goal_activated(_goal_id: String, _goal_data: Dictionary) -> void:
	_apply_goal_panel_text(_goal_data)
	_goal_panel_pending_reveal = true
	if _can_play_goal_panel_reveal_animation():
		_show_goal_panel_with_fade()
	else:
		goal_panel.hide()
		goal_panel.modulate.a = 0.0

func _pause_main_scene_bgm(reason: String) -> void:
	var key := reason.strip_edges()
	if key == "":
		return
	_main_scene_bgm_pause_reasons[key] = true
	_sync_main_scene_bgm_state()

func _resume_main_scene_bgm(reason: String) -> void:
	var key := reason.strip_edges()
	if key != "":
		_main_scene_bgm_pause_reasons.erase(key)
	_sync_main_scene_bgm_state()
...
func _clear_main_scene_bgm_fade_tween() -> void:
	if _main_scene_bgm_fade_tween and _main_scene_bgm_fade_tween.is_valid():
		_main_scene_bgm_fade_tween.kill()
	_main_scene_bgm_fade_tween = null

func _tween_main_scene_bgm_volume(target_volume_db: float, duration: float) -> Tween:
	if not is_instance_valid(bgm):
		return null
	_clear_main_scene_bgm_fade_tween()
	_main_scene_bgm_fade_tween = create_tween()
	_main_scene_bgm_fade_tween.tween_property(bgm, "volume_db", target_volume_db, duration)
	_main_scene_bgm_fade_tween.finished.connect(func() -> void:
		if _main_scene_bgm_fade_tween and not _main_scene_bgm_fade_tween.is_running():
			_main_scene_bgm_fade_tween = null
	, CONNECT_ONE_SHOT)
	return _main_scene_bgm_fade_tween

func _sync_main_scene_bgm_state() -> void:
	if not is_instance_valid(bgm):
		return
	var should_pause := not _can_resume_main_scene_bgm()
	bgm.stream_paused = should_pause
	if not should_pause and not bgm.playing:
		_clear_main_scene_bgm_fade_tween()
		bgm.volume_db = _main_scene_bgm_base_volume_db
		bgm.play()

func _on_activity_panel_visibility_changed() -> void:
	if not is_instance_valid(activity_panel_instance):
		return
	if activity_panel_instance.visible:
...
func _can_play_goal_panel_reveal_animation() -> bool:
	if not is_instance_valid(goal_panel) or not is_instance_valid(ui_panel):
		return false
	if not ui_panel.visible or ui_panel.modulate.a < 0.95:
		return false
	if _phone_mode_active:
		return false
	if is_instance_valid(mobile_interface_instance) and mobile_interface_instance.visible:
		return false
	return true

func _show_goal_panel_with_fade() -> void:
	if not is_instance_valid(goal_panel):
		return
	if _goal_panel_tween:
		_goal_panel_tween.kill()
	goal_panel.show()
	goal_panel.modulate.a = 0.0
	_goal_panel_pending_reveal = false
	_goal_panel_tween = create_tween()
	_goal_panel_tween.tween_property(goal_panel, "modulate:a", 1.0, 0.28)
	_goal_panel_tween.finished.connect(_refresh_guide_overlay_if_needed, CONNECT_ONE_SHOT)

func _hide_goal_panel() -> void:
	if not is_instance_valid(goal_panel):
		return
	if _goal_panel_tween:
		_goal_panel_tween.kill()
...
func _try_reveal_goal_panel_if_pending() -> void:
	if not _goal_panel_pending_reveal:
		return
	_update_goal_panel_ui()

func _is_affection_button_unlocked_by_guide() -> bool:
	var guide_manager := _get_guide_manager()
	if guide_manager == null:
		return true
	var default_unlocked := true
	if guide_manager.has_method("get_active_guide_id"):
		var active_guide_id := str(guide_manager.get_active_guide_id()).strip_edges()
		if active_guide_id == "schedule_onboarding_guide":
			default_unlocked = false
	if guide_manager.has_method("is_feature_unlocked"):
		return bool(guide_manager.is_feature_unlocked("main.affection", default_unlocked))
	return default_unlocked

func _update_affection_button_visibility() -> void:
	if not is_instance_valid(affection_button):
		return
	if not _is_affection_button_unlocked_by_guide():
		_affection_button_pending_reveal = false
		_hide_affection_button()
		return
	if _affection_button_pending_reveal:
		if _can_play_affection_button_reveal_animation():
			_show_affection_button_with_fade()
		else:
			_hide_affection_button()
		return
	if affection_button.visible:
		affection_button.modulate.a = 1.0
		return
	affection_button.show()
	affection_button.modulate.a = 1.0

func _can_play_affection_button_reveal_animation() -> bool:
	if not is_instance_valid(affection_button) or not is_instance_valid(ui_panel):
		return false
	if not ui_panel.visible or ui_panel.modulate.a < 0.95:
		return false
	if _phone_mode_active:
...
func _show_affection_button_with_fade() -> void:
	if not is_instance_valid(affection_button):
		return
	if _affection_button_tween:
		_affection_button_tween.kill()
	affection_button.show()
	affection_button.modulate.a = 0.0
	_affection_button_pending_reveal = false
	_affection_button_tween = create_tween()
	_affection_button_tween.tween_property(affection_button, "modulate:a", 1.0, 0.28)
	_affection_button_tween.finished.connect(_refresh_guide_overlay_if_needed, CONNECT_ONE_SHOT)

func _hide_affection_button() -> void:
	if not is_instance_valid(affection_button):
		return
	if _affection_button_tween:
		_affection_button_tween.kill()
	affection_button.modulate.a = 0.0
	affection_button.hide()

func _try_reveal_affection_button_if_pending() -> void:
	if not _affection_button_pending_reveal:
		return
	_update_affection_button_visibility()
...
func _show_affection_popup() -> void:
	...
	call_deferred("_refresh_guide_overlay_after_affection_layout")
	tween.finished.connect(_refresh_guide_overlay_if_needed, CONNECT_ONE_SHOT)
...
func _hide_affection_popup() -> void:
	...
	tween.chain().tween_callback(func():
		...
		_report_guide_action("close_affection_panel")
...
func has_scene_chat_mainline_available() -> bool:
	return not _get_active_story_topic_for_current_character().is_empty()

func _get_scene_chat_button() -> Button:
...
func _refresh_scene_chat_button_state() -> void:
	if not is_instance_valid(current_bg_scene):
		return
	if current_bg_scene.has_method("refresh_chat_button_state"):
...
func _on_date_pressed() -> void:
	...
	if FileAccess.file_exists(date_scene_path):
		...
		if date_scene_res:
			...
			date_scene.tree_exited.connect(func() -> void:
				_resume_main_scene_bgm("date_scene")
			, CONNECT_ONE_SHOT)
			_pause_main_scene_bgm("date_scene")
...
func _close_chat_panel(show_stats_toast: bool = true) -> void:
	...
	if _ui_tween:
		_ui_tween.kill()
	var guide_manager := _get_guide_manager()
	var should_reveal_affection_after_chat := false
	if guide_manager and guide_manager.has_method("get_current_step_id"):
		if str(guide_manager.get_current_step_id()) == "finish_first_chat_after_goal":
			should_reveal_affection_after_chat = true
	_report_guide_action("finish_first_main_chat_after_goal")
	if guide_manager and guide_manager.has_method("get_current_step_id"):
		if str(guide_manager.get_current_step_id()) == "explain_main_affection_button":
			should_reveal_affection_after_chat = true
	if should_reveal_affection_after_chat:
		_affection_button_pending_reveal = true
	_ui_tween = create_tween()
	_ui_tween.tween_property(ui_panel, "modulate:a", 1.0, 0.3)
...
func _remember_pending_main_scene_tts(text: String) -> void:
	var normalized_text := text.strip_edges()
	if normalized_text == "":
		return
	var current_count := int(_pending_main_scene_tts_counts.get(normalized_text, 0))
	_pending_main_scene_tts_counts[normalized_text] = current_count + 1

func _consume_pending_main_scene_tts(text: String) -> bool:
	var normalized_text := text.strip_edges()
	if normalized_text == "":
		return false
	var current_count := int(_pending_main_scene_tts_counts.get(normalized_text, 0))
	if current_count <= 0:
		return false
	if current_count == 1:
		_pending_main_scene_tts_counts.erase(normalized_text)
	else:
		_pending_main_scene_tts_counts[normalized_text] = current_count - 1
	return true
...
func _on_tts_success(audio_stream: AudioStream, text: String) -> void:
	if _story_mode_active:
		return
	var owned_by_main_scene := _consume_pending_main_scene_tts(text)
	if not owned_by_main_scene:
		return
	if audio_player:
...
func _ensure_main_bg_unlock_state() -> void:
	...
	var dirty := false
	var unlocked_ids: Array = GameDataManager.config.unlocked_main_bg_ids.duplicate()
	var default_ids: Array = []
	for entry in _main_bg_catalog:
		if bool(entry.get("default_unlocked", false)):
			var default_id := str(entry.get("id", "")).strip_edges()
			if default_id == "":
				continue
	...
	if unlocked_ids.is_empty():
		...
		unlocked_ids = default_ids
		dirty = true
	if dirty:
		GameDataManager.config.unlocked_main_bg_ids = unlocked_ids

	var current_bg_id := ""
	if GameDataManager.profile:
		current_bg_id = str(GameDataManager.profile.current_main_bg_id).strip_edges()
	if current_bg_id == "":
	...
	if GameDataManager.config.current_main_bg_id != current_bg_id:
		GameDataManager.config.current_main_bg_id = current_bg_id
		dirty = true
	if GameDataManager.profile and GameDataManager.profile.current_main_bg_id != current_bg_id:
		GameDataManager.profile.current_main_bg_id = current_bg_id
...
func _get_current_main_bg_id() -> String:
	if GameDataManager.profile:
		var profile_bg_id := str(GameDataManager.profile.current_main_bg_id).strip_edges()
		if profile_bg_id != "":
			return profile_bg_id
...
func _get_unlocked_main_bg_entries() -> Array:
	...
	for entry in _main_bg_catalog:
		...
		if GameDataManager.config and GameDataManager.config.has_method("is_main_background_unlocked"):
			is_unlocked = bool(GameDataManager.config.is_main_background_unlocked(final_id))
		elif GameDataManager.config:
			is_unlocked = GameDataManager.config.unlocked_main_bg_ids.has(final_id)
		final_entry["unlocked"] = is_unlocked
...
func _ready() -> void:
	...
	if stats_panel and not stats_panel.gui_input.is_connected(_on_stats_panel_gui_input):
		stats_panel.gui_input.connect(_on_stats_panel_gui_input)
	if goal_panel and not goal_panel.gui_input.is_connected(_on_goal_panel_gui_input):
		goal_panel.gui_input.connect(_on_goal_panel_gui_input)
	var goal_manager := _get_goal_manager()
	if goal_manager:
		if goal_manager.has_signal("goals_updated") and not goal_manager.goals_updated.is_connected(_on_goals_updated):
			goal_manager.goals_updated.connect(_on_goals_updated)
		if goal_manager.has_signal("goal_activated") and not goal_manager.goal_activated.is_connected(_on_goal_activated):
			goal_manager.goal_activated.connect(_on_goal_activated)
	var guide_manager := _get_guide_manager()
	if guide_manager and guide_manager.has_signal("feature_states_changed") and not guide_manager.feature_states_changed.is_connected(_on_guide_feature_states_changed):
		guide_manager.feature_states_changed.connect(_on_guide_feature_states_changed)
	if SceneTransitionManager and SceneTransitionManager.has_signal("transition_finished") and not SceneTransitionManager.transition_finished.is_connected(_on_scene_transition_finished):
		SceneTransitionManager.transition_finished.connect(_on_scene_transition_finished)
	...
	if is_instance_valid(bgm) and bgm.stream:
		if bgm.stream is AudioStreamMP3 or bgm.stream is AudioStreamOggVorbis:
			bgm.stream.loop = true
		_main_scene_bgm_base_volume_db = bgm.volume_db
	...
	var ready_guide_manager := _get_guide_manager()
	if ready_guide_manager and ready_guide_manager.has_method("on_main_scene_ready"):
		ready_guide_manager.on_main_scene_ready(self, did_just_finish_intro_story)
	...
	if story_post_event_manager and story_post_event_manager.has_method("process_timing"):
		call_deferred("_process_story_post_events_on_main_ready")
	await get_tree().create_timer(1.0).timeout
	if is_inside_tree():
...
func _can_trigger_idle_chatter() -> bool:
	...
	if is_instance_valid(activity_panel_instance) and activity_panel_instance.visible:
		return false
	var schedule_execution_panel := get_node_or_null("ScheduleExecutionPanel") as Control
	if is_instance_valid(schedule_execution_panel) and schedule_execution_panel.visible:
		return false
...
func _get_main_feature_fallbacks(feature_id: String) -> Array:
	if MAIN_FEATURE_ALIASES.has(feature_id):
		return MAIN_FEATURE_ALIASES[feature_id]
	return []

func _is_main_feature_unlocked_internal(feature_id: String, default_unlocked: bool = true) -> bool:
	var guide_manager := _get_guide_manager()
	if guide_manager == null or not guide_manager.has_method("is_feature_unlocked"):
		return default_unlocked
	var resolved_default := default_unlocked
	for fallback_id in _get_main_feature_fallbacks(feature_id):
		resolved_default = bool(guide_manager.is_feature_unlocked(str(fallback_id), resolved_default))
	return bool(guide_manager.is_feature_unlocked(feature_id, resolved_default))
...
func _refresh_guide_overlay_after_affection_layout() -> void:
...
func _on_guide_feature_states_changed() -> void:
	_update_affection_button_visibility()
	_update_main_feature_lock_states()
	_refresh_guide_overlay_if_needed()
...
func _on_scene_transition_finished(scene_path: String) -> void:
	if scene_path != "res://scenes/ui/main/main_scene.tscn":
		return
	if not is_inside_tree():
		return
	await get_tree().create_timer(0.22).timeout
...
func is_main_ui_ready_for_guide() -> bool:
	...
	if _phone_mode_active:
		return false
	if SceneTransitionManager and SceneTransitionManager.has_method("is_transitioning") and bool(SceneTransitionManager.is_transitioning()):
		return false
	if is_instance_valid(mobile_interface_instance) and mobile_interface_instance.visible:
		return false
	...
	if not ui_panel.visible:
		return false
	if ui_panel.modulate.a < 0.95:
		return false
	return true

func is_goal_panel_ready_for_guide() -> bool:
	if not is_main_ui_ready_for_guide():
		return false
	if not is_instance_valid(goal_panel):
		return false
	if not goal_panel.is_visible_in_tree():
		return false
	return goal_panel.modulate.a > 0.01

func is_interact_trigger_ready_for_guide() -> bool:
	if not is_main_ui_ready_for_guide():
		return false
	var interact_trigger_btn := _get_interact_trigger_button()
	if not is_instance_valid(interact_trigger_btn):
		return false
	if not interact_trigger_btn.is_visible_in_tree():
		return false
	return interact_trigger_btn.modulate.a > 0.01

func is_chat_button_ready_for_guide() -> bool:
	if not is_main_ui_ready_for_guide():
		return false
	var chat_btn := _get_scene_chat_button()
...
func is_affection_button_ready_for_guide() -> bool:
	...
	if not is_instance_valid(affection_button):
		return false
	if not affection_button.is_visible_in_tree():
		return false
	return affection_button.modulate.a > 0.01

func is_affection_panel_ready_for_guide() -> bool:
	if not is_instance_valid(affection_overlay) or not affection_overlay.visible:
		return false
	if not is_instance_valid(affection_popup_frame):
		return false
	if not affection_popup_frame.is_visible_in_tree():
		return false
	if affection_overlay.modulate.a < 0.95:
		return false
	if affection_popup_frame.scale.distance_to(Vector2.ONE) > 0.02:
		return false
	var target_control := _get_affection_panel_focus_target()
	if not is_instance_valid(target_control):
		return false
	var popup_rect := target_control.get_global_rect()
	return popup_rect.size.x > 32.0 and popup_rect.size.y > 32.0

func is_main_chat_topic_options_ready() -> bool:
	if not is_inside_tree():
		return false
...
func _build_main_action_cutout_polygon(rect: Rect2, slant_ratio: float) -> PackedVector2Array:
	...
	if rect.size.x <= 1.0 or rect.size.y <= 1.0:
		return polygon
	var top_inset := minf(rect.size.x - 1.0, rect.size.y * clampf(slant_ratio, 0.0, 1.0))
	polygon.append(Vector2(rect.position.x + top_inset, rect.position.y))
	polygon.append(Vector2(rect.end.x, rect.position.y))
	polygon.append(Vector2(rect.end.x, rect.end.y))
...
func get_main_action_focus_entry() -> Dictionary:
	var rect: Rect2 = _get_control_focus_rect(main_action_button)
	if rect.size.x <= 1.0 or rect.size.y <= 1.0:
		return {}
	var cutout_slant := 0.355
	var trapezoid_material := main_action_button.material as ShaderMaterial
	if trapezoid_material != null:
		var raw_slant: Variant = trapezoid_material.get_shader_parameter("slant")
		if raw_slant != null:
			cutout_slant = float(raw_slant)
	return {
		"rect": rect,
...
func get_affection_panel_focus_entry() -> Dictionary:
	var target_control := _get_affection_panel_focus_target()
...
func get_wechat_button_focus_entry() -> Dictionary:
	return _build_rounded_focus_entry(wechat_button, 18.0)

func get_interact_trigger_focus_entry() -> Dictionary:
	return _build_rounded_focus_entry(_get_scene_chat_button(), 20.0)
...
func _get_preferred_topic_option_target() -> Control:
	...
	for child in quick_options_container.get_children():
		...
		if not control_child.is_visible_in_tree():
			continue
		if child.has_method("get_option_kind") and str(child.get_option_kind()) == "story":
			return control_child
		if preferred_story_text != "" and child.has_method("get_option_text") and str(child.get_option_text()).strip_edges() == preferred_story_text:
			return control_child
	for child in quick_options_container.get_children():
...
func _on_stats_panel_gui_input(event: InputEvent) -> void:
	...
	var guide_manager := _get_guide_manager()
	if guide_manager and guide_manager.has_method("get_current_step_id"):
		if str(guide_manager.get_current_step_id()) == "explain_post_schedule_stats":
			_report_guide_action("click_main_stats")
			accept_event()

func _on_goal_panel_gui_input(event: InputEvent) -> void:
	if not (event is InputEventMouseButton):
		return
	var mouse_event := event as InputEventMouseButton
	if not mouse_event.pressed or mouse_event.button_index != MOUSE_BUTTON_LEFT:
...
func start_memory_revisit(revisit_data: Dictionary) -> void:
	...
	var btn := _get_scene_chat_button()
...
func start_proactive_greeting(prompt_type: String) -> void:
	...
	if deepseek_client.is_connected("idle_quote_completed", _on_proactive_greeting_generated):
		deepseek_client.idle_quote_completed.disconnect(_on_proactive_greeting_generated)
	if deepseek_client.is_connected("idle_quote_failed", _on_proactive_greeting_failed):
		deepseek_client.idle_quote_failed.disconnect(_on_proactive_greeting_failed)

	deepseek_client.idle_quote_completed.connect(_on_proactive_greeting_generated, CONNECT_ONE_SHOT)
	deepseek_client.idle_quote_failed.connect(_on_proactive_greeting_failed, CONNECT_ONE_SHOT)

	var char_id := "luna"
	if GameDataManager.profile:
		var current_character_id := str(GameDataManager.profile.current_character_id).strip_edges()
		if current_character_id != "":
			char_id = current_character_id

	deepseek_client.send_idle_quote_generation(char_id, {
		"scene_mode": "proactive_greeting",
...
func _check_afk_status() -> void:
	...
	var should_be_afk = is_minimized or is_covered_fullscreen or _is_desktop_pet_afk_active()
...
func _is_desktop_pet_afk_active() -> bool:
	...
	if not is_instance_valid(desktop_pet) or not desktop_pet.visible:
		return false
	if desktop_pet.has_method("is_main_window_parked"):
		return bool(desktop_pet.is_main_window_parked())
	return false
...
func _minimize_main_window_for_desktop_pet() -> void:
	...
	if root_window == null:
		return
	if is_instance_valid(desktop_pet_instance) and desktop_pet_instance.has_method("park_main_window_for_pet"):
		desktop_pet_instance.park_main_window_for_pet()
...
func _process_story_post_events_on_main_ready() -> void:
	if not is_inside_tree():
		return
	var story_post_event_manager := get_node_or_null("/root/StoryPostEventManager")
	if story_post_event_manager and story_post_event_manager.has_method("process_timing"):
...
func _on_main_action_pressed() -> void:
	...
	if _main_action_mode == "map":
		...
		if activity_panel_instance == null:
			...
			if not activity_panel_instance.visibility_changed.is_connected(_on_activity_panel_visibility_changed):
...
func _load_bg_scene(path: String) -> void:
	...
	if bg_packed:
		...
		var scene_chat_btn := _get_scene_chat_button()
		if scene_chat_btn:
