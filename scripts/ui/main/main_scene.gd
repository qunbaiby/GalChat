...
func _on_pomodoro_pressed() -> void:
    ...
    if not is_instance_valid(pomodoro_panel_instance):
        var pomodoro_panel_scene := load("res://scenes/ui/main/pomodoro_panel.tscn") as PackedScene
        if pomodoro_panel_scene == null:
            push_error("[MainScene] 无法加载番茄钟面板场景：res://scenes/ui/main/pomodoro_panel.tscn")
            if Engine.has_singleton("ToastManager"):
                ToastManager.show_system_toast("番茄钟面板加载失败", Color.RED)
            return
        pomodoro_panel_instance = pomodoro_panel_scene.instantiate()
