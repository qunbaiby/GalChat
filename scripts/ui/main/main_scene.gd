...
func is_interact_group_ready_for_guide() -> bool:
	if not is_main_ui_ready_for_guide():
		return false
	if not is_instance_valid(interact_group) or not interact_group.visible:
		return false
	return interact_group.modulate.a > 0.01
...
func is_chat_button_ready_for_guide() -> bool:
	...
	if not is_instance_valid(chat_button):
		return false
	return chat_button.is_visible_in_tree()
...
func get_interact_group_focus_entry() -> Dictionary:
	return _build_rounded_focus_entry(interact_group, 24.0)
...
func get_interact_trigger_focus_entry() -> Dictionary:
	return _build_rounded_focus_entry(_get_interact_trigger_button(), 20.0)

func get_chat_button_focus_entry() -> Dictionary:
	return _build_rounded_focus_entry(chat_button, 18.0)
...
func _on_interact_trigger_pressed() -> void:
	...
	if interact_group:
		...
		g_tween.finished.connect(_refresh_guide_overlay_if_needed, CONNECT_ONE_SHOT)
	call_deferred("_refresh_guide_overlay_if_needed")
	_report_guide_action("open_main_interact_group")
