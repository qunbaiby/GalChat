...
func _update_goal_panel_ui() -> void:
	if not is_instance_valid(goal_panel) or not is_instance_valid(goal_value_label):
		return
	...
	if not (goal_data is Dictionary) or (goal_data as Dictionary).is_empty():
		...
		_hide_goal_panel()
		return
	goal_value_label.text = _build_goal_panel_text(goal_data as Dictionary)
...
func _build_goal_panel_text(goal_data: Dictionary) -> String:
	var title := str(goal_data.get("title", "当前目标")).strip_edges()
...
func _on_goal_activated(_goal_id: String, _goal_data: Dictionary) -> void:
	goal_value_label.text = _build_goal_panel_text(_goal_data)
