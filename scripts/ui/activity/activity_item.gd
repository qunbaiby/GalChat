...
var _panel_style_normal: StyleBoxFlat
...
func _ready() -> void:
	...
	btn.mouse_entered.connect(_apply_hover_style)
	btn.mouse_exited.connect(_apply_normal_style)
	_capture_styles()
	_apply_normal_style()

func _capture_styles() -> void:
	_panel_style_normal = get_theme_stylebox("panel").duplicate() as StyleBoxFlat
	_panel_style_hover = _panel_style_normal.duplicate()
	_panel_style_hover.bg_color = Color(0.985, 0.992, 0.995, 1)
	_panel_style_hover.border_color = Color(0.67, 0.82, 0.9, 0.98)
	_panel_style_hover.shadow_color = Color(0.14, 0.24, 0.32, 0.12)
	_panel_style_hover.shadow_size = 10

	var clip_mask_panel: PanelContainer = $ClipMask
	_clip_style_normal = clip_mask_panel.get_theme_stylebox("panel").duplicate() as StyleBoxFlat
	_clip_style_hover = _clip_style_normal.duplicate()
	_clip_style_hover.bg_color = Color(0.985, 0.992, 0.995, 1)

	_progress_bg_normal = progress_bar.get_theme_stylebox("background").duplicate() as StyleBoxFlat
	_progress_bg_hover = _progress_bg_normal.duplicate()
	_progress_bg_hover.bg_color = Color(0.82, 0.88, 0.92, 1)

	_progress_fill_normal = progress_bar.get_theme_stylebox("fill").duplicate() as StyleBoxFlat
...
func _apply_normal_style() -> void:
	add_theme_stylebox_override("panel", _panel_style_normal)
...
func _apply_hover_style() -> void:
	add_theme_stylebox_override("panel", _panel_style_hover)
