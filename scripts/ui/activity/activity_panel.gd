...
func _init_category_tabs() -> void:
	...
	for cat in categories:
		var btn = null
		if category_tab_scene:
			btn = category_tab_scene.instantiate()
		else:
			btn = Button.new()
...
func _update_right_panel(profile) -> void:
	...
	if mood_diff != 0:
		...
		if mood_diff > 0:
			mood_bubble.add_theme_stylebox_override("normal", load("res://scenes/ui/activity/activity_panel.tscn::StyleBoxFlat_Bubble"))
		else:
			mood_bubble.add_theme_stylebox_override("normal", load("res://scenes/ui/activity/activity_panel.tscn::StyleBoxFlat_BubbleNeg"))
	else:
		mood_bubble.hide()
