...
const DYNAMIC_STYLES: Array[Dictionary] = [
    { "name": "单段回复", "weight": 35, "text": "【分段策略：单段回复】请将你的回答组织为连贯的一整段。要求：纯台词部分在 20 到 50 字之间，总字数不超过 65 字。【强制要求：请一次性发送完整内容，绝对不要使用 [SPLIT] 拆分段落】。注意必须同时包含括号括起来的动作/神态描写和真实台词对话！" },
...
const PET_DYNAMIC_STYLES: Array[Dictionary] = [
    { "name": "单段简短回复", "weight": 50, "text": "【分段策略：单段自然回复】请将你的回答组织为连贯的一整段。要求：纯台词部分在 10 到 30 字之间，保证说话内容的完整性和生动感。【强制要求：绝对不要使用 [SPLIT] 拆分！这段话中只能出现1个括号动作描述，且必须在句首！】" },
    { "name": "双段轻快交流", "weight": 50, "text": "【分段策略：双段轻快交流】请将你的回答分成 2 段发送。要求：【绝对强制】必须在两句话之间插入 [SPLIT] 作为分隔符！每个分段的台词应在 15 到 40 字之间，表现出聊天的停顿感。【绝对强制】每个被 [SPLIT] 隔开的段落里，最多只能包含1个括号动作描述，并且只能放在该段落的开头！例如：(动作)生动的台词内容 [SPLIT] (动作)补充的台词内容。" }
]

const MOBILE_CHAT_DYNAMIC_STYLES: Array[Dictionary] = [
    { "name": "单段手机回复", "weight": 50, "text": "【分段策略：单段回复】请将你的回答组织为连贯的一整段。要求：纯台词部分在 10 到 50 字之间，总字数不超过 80 字。【绝对强制要求：这是手机聊天界面的对话，请直接输出对话文本，绝对不要使用括号添加任何动作、神态或心理描写！绝对不要使用 [SPLIT] 拆分段落！】" },
