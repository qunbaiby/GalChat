...
func build_system_prompt(profile: CharacterProfile, template_name: String = "default_chat", player_message: String = "", query_embedding: Array = [], memory_manager_override = null, summary_channel: String = "", prompt_access_context: Dictionary = {}) -> String:
	...
	var player_name = ""
	if template_name == "desktop_pet" and GameDataManager and GameDataManager.config:
		player_name = str(GameDataManager.config.player_nickname).strip_edges()
	if player_name.is_empty():
		player_name = str(profile.player_title).strip_edges()
	if player_name.is_empty():
		player_name = "哥哥" if template_name == "desktop_pet" else "老师"
...
func build_end_chat_prompt(profile: CharacterProfile, recent_history: String) -> String:
	...
	var flavor_label = _resolve_flavor_label(profile)
	if template == "":
		return "【系统提示：玩家想要结束本次对话。请根据你们当前的关系状态（亲密度：%.1f，信任度：%.1f，风味：%s），并结合以下刚才聊天的上下文，自然表示这次先聊到这里、之后有机会再聊。只结束当前对话，不要开启或延伸新话题。你和玩家住在同一栋房子里，不得说自己或玩家要回家、离开这栋房子。注意：1. 只能回复单句结束语。2. 不要输出这段系统提示，直接以%s的口吻说话。】\n[近期聊天上下文]\n%s" % [profile.intimacy, profile.trust, flavor_label, profile.char_name, recent_history]
