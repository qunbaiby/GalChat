...
func start_default_guide_if_needed(trigger_source: String = "") -> bool:
	...
	if not is_guide_opted_in():
		can_start = false
		reason = "guide_not_opted_in"
	elif not _guide_defs.has(DEFAULT_GUIDE_ID):
		can_start = false
		reason = "default_guide_missing"
	elif is_guide_completed(DEFAULT_GUIDE_ID):
		can_start = false
		reason = "default_guide_completed"
	elif str(_state.get("active_guide_id", "")).strip_edges() != "":
		can_start = false
		reason = "active_guide_exists"
	else:
		var guide_data_preview: Dictionary = _guide_defs.get(DEFAULT_GUIDE_ID, {})
		if trigger_source == "intro_story" and not bool(guide_data_preview.get("auto_start_after_intro", false)):
			can_start = false
			reason = "intro_story_auto_start_disabled"
	if not is_guide_opted_in():
		return false
...
func start_guide(guide_id: String) -> bool:
	...
	var exists := _guide_defs.has(normalized_guide_id)
	var completed := is_guide_completed(normalized_guide_id)
...
func _append_focus_rect_result(target_rects: Array[Rect2], focus_result: Variant) -> void:
...
func _is_step_scene_ready(step_data: Dictionary) -> bool:
	...
	if requires_scene == "main":
		return is_instance_valid(_resolve_main_scene())
	...
	if requires_scene == "schedule_execution":
		...
		return is_instance_valid(execution_panel) and (not (execution_panel is Control) or (execution_panel as Control).visible)
...
func _resolve_step_focus_result(step_data: Dictionary) -> Variant:
			...
			match target_mode:
				"category_tabs":
					if activity_panel.has_method("get_category_tabs_focus_rect"):
						return activity_panel.get_category_tabs_focus_rect()
				"activity_list":
					if activity_panel.has_method("get_activity_list_focus_rect"):
						return activity_panel.get_activity_list_focus_rect()
				"tabs_and_list":
					if activity_panel.has_method("get_tabs_and_list_focus_rect"):
						return activity_panel.get_tabs_and_list_focus_rect()
				"schedule_slots":
					if activity_panel.has_method("get_schedule_slots_focus_rect"):
						return activity_panel.get_schedule_slots_focus_rect()
	...
	var padding := maxf(0.0, float(step_data.get("highlight_padding", 10.0)))
