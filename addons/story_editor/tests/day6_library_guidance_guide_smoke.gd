...
func _run() -> void:
	...
	_expect(not guide_manager.start_day6_library_guide_if_needed(), "第五剧情日错误启动了周六图书馆引导。")
	...
	game_data_manager.story_time_manager.current_day_offset = 5
	_expect(guide_manager.start_day6_library_guide_if_needed(), "进入第六剧情日后没有启动周六图书馆引导。")
	...
	_expect(str(tutoring_courses_step.get("target_mode", "")) == "course_list" and str(tutoring_courses_step.get("text", "")).contains("五次课业指导"), "课程列表引导目标或文案不正确。")
