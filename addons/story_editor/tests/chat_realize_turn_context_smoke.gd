...
func _run() -> void:
	...
	_expect(dialogue_manager_source.contains('"story_dialogue_player"'), "Story 普通玩家对话没有独立回调渠道。")
	...
	_expect(dialogue_manager_source.contains("if _uses_ai_player_option_layer() and is_instance_valid(ai_player_options_container):") and dialogue_manager_source.contains("dialogue_panel.show_ai_player_options()"), "Story AI 主线与日常对话动态选项没有切换到 AiPlayerOptionLayer。")
	_expect(dialogue_manager_source.contains("return _guided_ai_chat_active or _is_embedded_daily_chat()"), "日常对话没有与 AI 主线共用玩家选项层。")
	_expect(dialogue_manager_source.contains('dialogue_panel.set_ai_player_option_status("Luna正在思考中")') and dialogue_manager_source.contains('dialogue_panel.set_ai_player_option_status("Luna正在讲话")'), "Story AI 主线期间 AiPlayerOptionLayer 没有持续展示角色状态。")
	_expect(dialogue_manager_source.contains("quick_options_container,\n\t\t_embedded_topic_options,\n\t\t_on_embedded_topic_selected"), "Story AI 主线首话题没有保留在 QuickOptionLayer。")
