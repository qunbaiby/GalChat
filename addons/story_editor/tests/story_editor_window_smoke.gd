...
func _run() -> void:
	...
	if editor != null:
		...
		_expect(content_labels == ["AI 约会", "手机 AI", "剧情引用", "Guide Flow", "入口调度"], "内容库仍包含已移除的心事入口或菜单顺序异常。")
		_expect(editor.get_node_or_null("ConcernAIWorkbench") == null, "主编辑器仍实例化 AI 心事工作台。")
		_expect(editor.get_node_or_null("Root/Toolbar/ConcernAIButton") == null, "主编辑器仍保留 AI 心事工具栏按钮。")
