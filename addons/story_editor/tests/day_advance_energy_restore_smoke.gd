...
func _run() -> void:
	...
	_expect(game_data_manager != null and game_data_manager.profile != null and game_data_manager.story_time_manager != null, "跨日精力测试依赖未初始化。")
	...
	_expect(time_manager.current_hour == 6 and time_manager.current_minute == 0, "跨日后没有统一推进到次日 06:00。")
	_expect(profile.current_energy == profile.max_energy, "跨日后内存中的精力没有回满。")
	_expect(int(update_count[0]) > 0, "跨日恢复精力后没有通知 UI 刷新。")
	...
	_expect(saved_data is Dictionary and int((saved_data as Dictionary).get("current_energy", -1)) == profile.max_energy, "跨日恢复后的精力没有写入角色存档。")
