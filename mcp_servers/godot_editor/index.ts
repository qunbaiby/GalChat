...
let managedGodotProcess: ChildProcess | null = null;
...
async function requestJson(pathname: string, options?: RequestOptions): Promise<JsonObject> {
...
function toolSuccess<T extends JsonObject>(data: T) {
  return {
...
function getGodotDataRoot(): string {
...
async function resolveEditorLogPath(projectPath: string, explicitLogPath?: string): Promise<string> {
  const directCandidates = [explicitLogPath, DEFAULT_EDITOR_LOG_PATH].filter(
...
function tailTextLines(text: string, maxLines: number) {
...
server.tool(
  "godot_editor_status",
  "检查 Godot 编辑器插件是否在线，并返回当前项目、场景和选择状态。",
  {},
  async () => {
...
server.tool(
  "godot_project_open",
  "启动 Godot 编辑器并打开指定项目。优先使用传入的 executable_path，其次使用 GODOT_EXECUTABLE 环境变量。",
  {
...
server.tool(
  "godot_project_run",
  "通过当前已连接的 Godot 编辑器运行主场景、当前场景或自定义场景。",
  {
...
server.tool(
  ...
  "获取 Godot 当前正在编辑的场景信息，包括场景路径、根节点和子节点数量。",
  {},
  async () => {
    try {
...
server.tool(
  ...
  "设置当前编辑场景中指定节点的属性值。支持 string、int、float、bool、json、vector2、vector2i、vector3、vector3i、color、node_path。",
  {
...
server.tool(
  "godot_save_scene",
  "保存当前编辑场景。若传入 scene_path，则另存为该路径。",
  {
...
server.tool(
  ...
  "读取最近的 Godot 运行日志，默认定位到当前项目的 user://logs/godot.log。",
  {
    project_path: z.string().optional().describe("Godot 项目路径，默认使用当前项目路径"),
